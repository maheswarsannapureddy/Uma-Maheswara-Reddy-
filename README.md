# Uma Maheswara Reddy Sannapureddy
...
This project demonstrates a robust and flexible API server built with **FastAPI** and **LangChain**, integrated with the open-source **Ollama (Llama2)** model for advanced natural language processing. The server generates informative content and creative outputs, showcasing its versatility and seamless interaction between modern web frameworks and cutting-edge language models.
...
- **Environment Setup**: Secured using `.env` to manage sensitive API keys like `LANGCHAIN_API_KEY`.
- **Model**: Initialized with **Llama2**, handling tasks like:
  - `Prompt 1`: Generates detailed information about a topic.
  - `Prompt 2`: Creates a short, creative song about a topic.
- **Endpoints**:
  - `/information`: Accepts a topic and returns detailed information.
  - `/song`: Accepts a topic and returns a creative song.

### Frontend: Streamlit Application
- **Features**: Simple and interactive interface for entering topics.
- **API Interaction**: Sends POST requests to FastAPI endpoints, displaying real-time results.

---

## Key Features
- **Integration of Open-Source Tools**: Combines **FastAPI**, **LangChain**, and **Ollama** for an efficient and scalable solution.
- **Extensibility**: Easily add more prompts and endpoints.
- **User-Friendly Interface**: Streamlit ensures a seamless interaction with the backend API.

---

## Future Work
- **Add Models**: Expand with additional NLP models.
- **New Prompts**: Broaden the use cases.
- **Optimization**: Enhance performance for large-scale deployments.
...
1. Clone the repository:
   ```bash
   git clone https://github.com/your-username/API-Server-with-LangChain-Ollama-and-OpenAI.git
